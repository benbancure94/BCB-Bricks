(function(_window_) {
	...
	var _games = [
	{
		...
		load: function() {
			...
			function moveTank(direction) {
				var tankDirection = tankTile.getDirection();
				var tankLocation = tankTile.getLocation();

				if (tankDirection == direction) {
					switch(tankDirection) {
						case "Left":
							if(tankLocation.x > 0) tankLocation.x--;
							break;
						case "Right":
							if(tankLocation.x < 17) tankLocation.x++;
							break;
						case "Up":
							if(tankLocation.y > 0) tankLocation.y--;
							break;
						case "Down":
							if(tankLocation.y < 7) tankLocation.y++;
							break;										
					}
					tankTile.setLocation(tankLocation.x, tankLocation.y);
				}
				else {
			...
			function moveRoad() {
				...
				if (lastCarLocation.x > 12 && lastCarLocation.y == myCar.getLocation().y) {
					...
					if (_road1.first().getLocation().x == 2) {
						...
						var _r2 = new BrickObject({ tiles: [{x: 0, y: 0}, {x: 1, y: 0}, {x: 2, y: 0}], color: "red" });
	...
	function BrickObject(params) {
		...
		var originX = params.originX == undefined ? 0: params.originX;
		var originY = params.originY == undefined ? 0: params.originY;
