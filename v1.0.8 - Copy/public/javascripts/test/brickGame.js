...
function BrickGame() {
	...
	var GameSound = new function() {
		...
		var music = {
			current: 0,
			audios: [
				new Audio(soundBaseURL + "/opening.wav"),
	...
	var BrickGameModel1 = new function() {
		// DECLARATIONS
		...
		// this.setLevel(gameData.level);
		// this.setSpeed(gameData.speed);
	};
	var BrickGameModel = new function() {
		// DECLARATIONS
		var mode = 1, score = 0, gameNumber = 0, lives = 0, speedInMillis = 1000;
		var brickTiles = [], selectedGame = {};
		var isPaused = false;

		// METHODS
		this.getLevel = function() { return gameData.level; };
		this.setLevel = function(_level) { gameData.level = _level; levelNumber.innerText =_level; }
		this.getSpeed = function() { return gameData.speed; }
		this.setSpeed = function(_speed) { gameData.speed = _speed; speedNumber.innerText = _speed; }
		this.getGameNumber = function() { return gameNumber; }
		this.setGameNumber = function(_gameNumber) { gameNumber = _gameNumber; }
		this.getScore = function() { return score; }
		this.resetScore = function() { score = 0; scoreNumber.innerText = score; }
		this.getSpeedInMillis = function() { return speedInMillis; }
		this.setSpeedInMillis = function(_speedInMillis) { speedInMillis = _speedInMillis }
		this.getSelectedGame = function() { return selectedGame; }
		this.setSelectedGame = function(_selectedGame) { 
			selectedGame = _selectedGame; 
			scoreNumber.innerText = gameData.highScores[gameNumber];
		}
		this.changeSoundVolume = function(volume) {
			if(volume == 1 || volume == 0.75) {
				soundIcon.classList.remove("fa-volume-off", "fa-volume-down");
		...
		this.score = function() {
			...
			if(gameData.highScores[gameNumber] < score) {
				selectedGame.score = score;
				gameData.highScores[gameNumber] = score;
			}
		};
		this.togglePause = function() { isPaused = !isPaused; if (isPaused) pauseIcon.classList.remove("deact"); else pauseIcon.classList.add("deact"); }
	...
	window.onkeydown = function() { GameKeys.onkeydown(event); }
	window.onkeyup = function() { GameKeys.onkeyup(event); }
...
(function() {
	...
	gameDataRequest.open("GET", "http://localhost:8080/getGameData", true);
