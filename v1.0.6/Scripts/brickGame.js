(function(_window_) {
	...
	},
	{
		...
		load: function($game) {
			...
			var myCar = $game.newBrickObject({ name: "carTile", brickLocation: { x: 16, y: 5 } });
			var moveRoadAnim = $game.newTimer({ func: moveRoad, interval: speedInMillis });
			$game.newBrickObject({ name: "twentyByOne", brickLocation: { x: 0, y: 0 } });
			$game.newBrickObject({ name: "twentyByOne", brickLocation: { x: 0, y: 9 } });
			...
			function loadIndividualObstacle(x, topObstacleCount) {
				...
				for (var i = 1; i < topObstacleCount + 1; i++)
					_o1.push($game.newBrickObject({ name: "sixByOne", brickLocation: { x: x, y: i } }));
				for (var i = 8; i > 8 - bottomObstacleCount; i--)
					_o2.push($game.newBrickObject({ name: "sixByOne", brickLocation: { x: x, y: i } }));
			...
			function moveRoad() {
				var willCollide = $game.willBeCollidedBy(myCar, "Left").length > 0;
				var willOverlap = $game.willBeOverlappedBy(myCar).length > 0;
				if (willCollide || willOverlap) {
					GameSound.explosion(); $game.stop();
					...
					if ((lastTopObstacle.first() != undefined && lastTopObstacle.first().isGottenOutOfScreen()) || 
						...
						if (lastTopObstacle != undefined)
							for (var i = 0; i < lastTopObstacle.length; i++) $game.disappear(lastTopObstacle[i]);
						if (lastBottomObstacle != undefined)
							for (var i = 0; i < lastBottomObstacle.length; i++) $game.disappear(lastBottomObstacle[i]);
			...
			this.keydownfunctions = {
				onLeft: function() { },
				onRight: function() { },
				onTop: function() { var y = myCar.getLocation().y; myCar.setLocation(16, y - 1) },
				onBottom: function() { var y = myCar.getLocation().y; myCar.setLocation(16, y + 1); },
