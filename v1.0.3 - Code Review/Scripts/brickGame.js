(function(_window_) {
	...
	function BrickObject(tiles, color) {
		...
		tiles = new (function() { return tiles; })();
		...
		this.setLocation = function(x, y, _tiles) {
			...
			if(_tiles != undefined) tiles = _tiles;
		...
		this.hasTile = function(x, y) {
			...
			var hasTile = false;
			if(X != undefined && Y != undefined) {
				while(t < tiles.length && hasTile == false) {
					if(tiles[t].x + X == x && tiles[t].y + Y == y) hasTile = true;
					t++;
				}
			}
			return hasTile;
