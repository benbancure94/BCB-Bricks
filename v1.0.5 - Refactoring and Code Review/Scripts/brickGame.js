(function(_window_) {
	...
	[
		...
		{
			name: "quadrupleTiles",
			tiles: [{ x: 0, y: 0 }, { x: 0, y: 1 }, { x: 0, y: 2 }, { x: 0, y: 3 } ]
		},
		{
			name: "carTile",
			tiles: [
		...
		{
			name: "soldierTile",
			tiles: [{ x: 0, y: 1 }, { x: 1, y: 0 }, { x: 1, y: 1 }, { x: 1, y: 2 }]
		},
		{
	...
	var GameSound = new function() {
		...
		this.pause = function() {
			if (audio == undefined) throw new Error("Error: undefined");
		};
		this.stop = function() {
			if (audio == undefined) throw new Error("Error: undefined");
		};
	...
	function changeTileColor(x, y, color) {
		var xCharacter = brickObjectTiles[x].name;
		var yCharacter = brickObjectTiles[y].name;
	...
	var Game = new function() {
		...
		this.newBrickObject = function(params) {
			...
			params.tiles = params.tiles == undefined ? brickTiles.tiles: params.tiles;
	...
	},
	{
		...
		load: function($game) {
			...
			var level = brickGameModel.getLevel();
			var speedInMillis = brickGameModel.getSpeedInMillis();
			var soldierObject = $game.newBrickObject({ 
				name: "soldierTile",
				brickLocation: { x: 18, y: 4 },
				willPassThroughSide: true
			});
			...
			function newEnemyRow(i) {
				var enemies = [];
				for (var j = 0; j < 10; j++) 
					if(Math.floor(Math.random() * 10) % 2 == 0) enemies.push({ x: 0, y: j });
	...
	},
	{
		...
		load: function($game) {
			...
			var pinball = $game.newBrickObject({ 
				name: "singleTile", 
				color: "Green",
				brickLocation: { x: 18, y: 5 }
			});
			...
			if(level == 9) {
				...
				params = {
					name: "pinballTileLevel" + level.toString()
				}
			}
	...
	},
	{
		...
		load: function($game) {
			...
			// FUNCTIONS
			function addCrosser() {
				crosser = $game.newBrickObject({ name: "singleTile", color: "Green" , brickLocation: { x: 19, y: 5 }});
			}
			function loadWalls() {
				for (var i = 2; i < 19; i += 2) {
					$game.newBrickObject({
						name: "verticalLine",
						brickLocation: { x: i, y: 0 }
					});
				}
			}
					...
					case "Left":
						...
						if (location.x == 0) {
							...
							if(bumpedCrosser == null || bumpedCrosser == undefined) {
								...
								GameSound.explosion();
								$game.blinkBrickObjects({
									brickObjects: [bumpedCrosser, crosser],
									interval: 400,
									count: 3,
									endFunction: $game.gameOver
								});
	...
	},
	{
		...
		mode: 7,
		score: 0,
		speedTimeout: [2000, 1900, 1800, 1700, 1600, 1500, 1400, 1300, 1200, 1100],
		load: function($game) {
			// DECLARATIONS
			var position = 4;
			...
			function movePinballTile() {
				...
				if (location.x + brickWidth == 17) gameOver();
					...
					var fallTileAnim = $game.newTimer({ func: function() { 
						fallTile(fallenTile) 
					}, interval: 100 });
					...
					fallenTile = $game.newBrickObject({
						name: "singleTile",
						brickLocation: { x: fallenTile.screenX, y: fallenTile.screenY },
						color: "red",
						onRemove: fallTileAnim.stop
					});
			...
			function fire() {
				var fallenTile = fallenTiles.filter(function(ft) { 
					return ft.getLocation().y == position + 1;
				}).sort(function(t1, t2) {
					return t1.x - t2.x;
				}).first();
				...
				else if (pinballTile.removeSideTile("Right", position + 1)) {
					$game.score();
				}
	...
	// },
	{
		...
		load: function($game) {
			...
			function addCar() {
				return newCar(-4, ((Math.floor(Math.random() * 100) % 3) * 3) + 1);
			}
			function loadRoadAndCars() {
			...
			function moveRoad() {
				...
				if (willCollide || willOverlap) {
					...
					if (_lastRoad1.isGottenOutOfScreen()) {
						$game.disappear(_lastRoad1); _road1.pop();
					}
					for (var i = 0; i < cars.length; i++) {
						var location = cars[i].getLocation();
						cars[i].setLocation(location.x + 1, location.y);
	...
	function BrickObject(params) {
		...
		this.isGottenOutOfScreen = function() {
			var brickSize = _getSize();
			console.log(brickTileName + " gotten out");
		...
		function _removeTile(x, y) {
			...
			if(isTileRemoved) {
				changeTileColor(x, y, tileToBeRemoved.backColor);
		...
		this.getSideTiles = function(direction) {
			...
			switch(direction) {
				case "Right":
					edgeTiles = tiles.sort(function(t1, t2) {
						return t1.x - t2.x;
					});
					edgeTiles = edgeTiles.filter(function(t) {
						return t.x == edgeTiles.last().x;
					});
					break;
			}
			return edgeTiles;
	...
	function Timer(params) {
		...
		var _interval = params.interval == undefined ? 0: params.interval;
