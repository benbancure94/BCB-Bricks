(function(_window_) {
	...
	_window_.onkeydown = function() {
		GameKeys.onkeydown(event);
	}
	_window_.onkeyup = function() {
		GameKeys.onkeyup(event);
	}
	...
	var brickGameModel = new function() {
		...
		this.togglePause = function() {
			isPaused = !isPaused;
			if (isPaused) {
				pauseIcon.classList.remove("deact");
			}
			else {
				pauseIcon.classList.add("deact");
			}
		} 
	}
	...
	function changeSoundVolume() {
		...
		GameKeys.s.onkeydown.allowRepeat(true);
		GameKeys.s.onkeydown.setEnabled(true);
	}
	...
	function pause() { 
		brickGameModel.togglePause();
		timers.forEach(function(t) { if (t.isRunning()) t.pause(); }); 
	}
	function stop() { timers.forEach(function(t) { t.stop(); }); }
	function resume() { 
		brickGameModel.togglePause();
		timers.forEach(function(t) { if (t.isPaused()) t.start(); }); 
	}
	...
	var Marquee = function(params) {
		...
		//GameKeys.keydownfunctions.all(exitFunction);

		GameKeys.all.onkeydown.func(exitFunction);
		GameKeys.all.onkeydown.setEnabled(!isKeyLocked);
		GameKeys.all.onkeydown.allowRepeat(true);

		GameKeys.all.onkeyup.clear();

		//GameKeys.keypress.clear();
		// GameKeys.keyup.clear();
	}
