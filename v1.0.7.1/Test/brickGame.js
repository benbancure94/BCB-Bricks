(function(_window_) {
	...
	var WindowKeys = new function() {
		...
		this.onkeydown = function(event) {
			...
			if((keydownfunction.allowRepeat || 
				!event.repeat) &&
				keydownfunction.enabled) { console.log(keydownfunction.test); keydownfunction.func(); }
		};
		this.onkeyup = function(event) {
			...
			console.log(keyupfunction.test);
			keyupfunction.func();
		...
		function clearKeyUp() {
			for (var k = 0; k < availableKeyNames.length; k++) {
				keyfunctions[availableKeyNames[k]].onkeyup = {
					func: function() {}, test: availableKeyNames[k] + " key up"
				}
			}
		}
	}
	...
	function changeSoundVolume() {
		WindowKeys.s.onkeydown.func(function() { 
			brickGameModel.changeSoundVolume();
			GameSound.select();
		});
		WindowKeys.s.onkeydown.allowRepeat(true);
		WindowKeys.s.onkeydown.setEnabled(true);
	}
	...
	function pause() { timers.forEach(function(t) { if (t.isRunning()) t.pause(); }); }
	function stop() { timers.forEach(function(t) { t.stop(); }); }
	function resume() { timers.forEach(function(t) { if (t.isPaused()) t.start(); }); }
	...
	var Marquee = function(params) {
		...
		WindowKeys.all.onkeydown.func(exitFunction);
		WindowKeys.all.onkeydown.setEnabled(!isKeyLocked);
		WindowKeys.all.onkeydown.allowRepeat(true);

		WindowKeys.all.onkeyup.clear();
	...
	var GameProperties = function() {
		...
		var upkeydown = WindowKeys.up.onkeydown;
		var downkeydown = WindowKeys.down.onkeydown;
		var leftkeydown = WindowKeys.left.onkeydown;
		var rightkeydown = WindowKeys.right.onkeydown;
		var spacekeydown = WindowKeys.space.onkeydown;
		var enterkeydown = WindowKeys.enter.onkeydown;
	...
	var Game = new function() {
		...
		var spacekeydown = WindowKeys.space.onkeydown;
		var enterkeydown = WindowKeys.enter.onkeydown;
		var defaultkeydown = WindowKeys.default.onkeydown;

		var upkeyup = WindowKeys.up.onkeyup;
		var downkeyup = WindowKeys.down.onkeyup;
		var leftkeyup = WindowKeys.left.onkeyup;
		var rightkeyup = WindowKeys.right.onkeyup;
		var spacekeyup = WindowKeys.space.onkeyup;
		var enterkeyup = WindowKeys.enter.onkeyup;
		var defaultkeyup = WindowKeys.default.onkeyup;
		...
		var lockKey = function(locked) {
			upkeydown.setEnabled(!locked);
			downkeydown.setEnabled(!locked);
			leftkeydown.setEnabled(!locked);
			rightkeydown.setEnabled(!locked);
			spacekeydown.setEnabled(!locked);
		};
