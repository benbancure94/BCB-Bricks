(function(_window_) {
	...
	var WindowKeys = new function() {
		...
		var isKeydownEnabled = true;

		function isKeyEnabled(key) {
			return lockedkeys.keydown.filter(function(kd) { return kd == key }).first() == undefined;
		}

		this.keydownfunctions = {
			setKey: function(key, func, allowRepeat) {
				if (isKeydownEnabled && isKeyEnabled(key)) {
					var availableKeyName = Object.keys(availableKeyNames).filter(function(k) {
						return availableKeyNames[k] == key;
					}).first();
					if (availableKeyName == undefined) throw new Error("Invalid key");
					if (allowRepeat == undefined) allowRepeat = true;
					keyfunctions.keydown[key] = { func: func, allowRepeat: allowRepeat };
				}
			}, // ok
			deleteKey: function(key) {
				if (isKeydownEnabled) {
					...
					if (availableKeyName == undefined) throw new Error("Invalid key");
					delete keyfunctions.keydown[key];
				}
			}, // ok
			setKeyEnabled: function(key, isEnabled) {
				var availableKeyName = Object.keys(availableKeyNames).filter(function(k) {
					return availableKeyNames[k] == key;
				})[0];
				if (availableKeyName == undefined) throw new Error("Invalid key");
				var lockedKey = lockedkeys.keydown.filter(function(kd) { return kd == key }).first();
				if (lockedKey == undefined && !isEnabled) {
					lockedkeys.keydown.push(key);
				}
				else if (lockedKey != undefined && isEnabled) {
					lockedkeys.keydown.splice(lockedkeys.keydown.indexOf(lockedKey), 1);
				}
			}, // ok
			defaultKeys: function(func, allowRepeat) {
				if (allowRepeat == undefined) allowRepeat = true;
				if (isKeydownEnabled) keyfunctions.keydown.default = { func: func, allowRepeat: allowRepeat };
			}, // ok
			setEnabled: function(isEnabled) {
				isKeydownEnabled = isEnabled;
			}, // ok
			all: function(func, allowRepeat) {
				if (allowRepeat == undefined) allowRepeat = true;
				if (isKeydownEnabled) {
					var keys = Object.keys(keyfunctions.keydown);	
					for (var i = 0; i < keys.length; i++) {
						if(keys[i] == "default") keyfunctions.keydown.default = { func: func, allowRepeat: allowRepeat };
						else delete keyfunctions.keydown[keys[i]];
					}
				}
			}, // ok
			clear: function() {
				var keys = Object.keys(keyfunctions.keydown);	
				for (var i = 0; i < keys.length; i++) {
					if(keys[i] == "default") keyfunctions.keydown.default = { func: function() {}, allowRepeat: false };
					else delete keyfunctions.keydown[keys[i]];
				}
			},
			clearBut: function(...excludedKeys) {
				var keys = Object.keys(keyfunctions.keydown);	
				for (var i = 0; i < keys.length; i++) {
					var isExcluded = excludedKeys.filter(function(ek) { return ek == keys[i]; }).length > 0;
					if (!isExcluded) {
						if(keys[i] == "default") keyfunctions.keydown.default = { func: function() {}, allowRepeat: false };
						else delete keyfunctions.keydown[keys[i]];
					}
			...
			deleteAll: function() {
				var keys = Object.keys(keyfunctions.keydown);	
				for (var i = 0; i < keys.length; i++) {
					if(keys[i] != "default") delete keyfunctions.keydown[keys[i]];
				}
			},
			deleteAllBut: function(...excludedKeys) {
				var keys = Object.keys(keyfunctions.keydown);	
				for (var i = 0; i < keys.length; i++) {
					var isExcluded = excludedKeys.filter(function(ek) { return ek == keys[i]; }).length > 0;
					if (!isExcluded) {
		...
		this.keyupfunctions = {
			setKey: function(key, func) {
				if (isKeydownEnabled && isKeyEnabled(key)) {
					...
					if (availableKeyName == undefined) throw new Error("Invalid key");
					keyfunctions.keyup[key] = func;
				}
			},
			clear: function() {
				var keys = Object.keys(keyfunctions.keyup);	
				for (var i = 0; i < keys.length; i++) {
					if(keys[i] == "default") keyfunctions.keyup.default = function() {};
					else delete keyfunctions.keyup[keys[i]];
				}
			},
			deleteAll: function() {
				var keys = Object.keys(keyfunctions.keyup);	
				for (var i = 0; i < keys.length; i++) {
					if(keys[i] != "default") delete keyfunctions.keyup[keys[i]];
				}
			},
		}
		...
		this.onkeydown = function(event) {
			...
			var key = availableKeyNames[eventkey];
			if (isKeydownEnabled && isKeyEnabled(key)) {
				var keyFunction = keyfunctions.keydown[key];
				var kdf = function() {};
				if (key == undefined || keyFunction == undefined) { // if key function not found by given key
					kdf = keyfunctions.keydown["default"];
				}
				else {
					kdf = keyfunctions.keydown[key];
				}

				if(kdf.allowRepeat || !event.repeat) kdf.func();
			}
		};
		...
		this.onkeyup = function(event) {
			var eventkey = event.key.toLowerCase();
			var key = availableKeyNames[eventkey];
			var keyFunction = keyfunctions.keyup[key];
			if (key == undefined || keyFunction == undefined) { // if key function not found by given key
				kuf = keyfunctions.keyup["default"];
			}
			else {
				kuf = keyfunctions.keyup[key];
			}
			kuf.func();
		}
	}
	...
	function changeSoundVolume() {
		WindowKeys.keydownfunctions.setKey("s", function() { 
	...
	function removeBrickObjects(brickObjects) {
		for (var i = 0; i < brickObjects.length; i++) {
			brickObjects[i].remove();
		}
	}
	function removeBrickObjectsBut(...bos) {
		...
		for (var i = len - 1; i >= 0; i--) { 
			var brickObject = bos.filter(function(bo) {
				if (brickObjects.indexOf(bo) == i) {
					console.log(brickObjects.indexOf(bo), i);
				}
				return brickObjects.indexOf(bo) == i;
			}).length;
	...
	var Marquee = function(params) {
		...
		WindowKeys.keydownfunctions.all(exitFunction);

		//WindowKeys.keypress.clear();
		// WindowKeys.keyup.clear();
	}
	
	// START PAGE
	_window_.onload = function() {
		loadTiles(); loadLifeTiles();
	...
	var GameProperties = function() {
		...
		changeSoundVolume();
		WindowKeys.keydownfunctions.setKey("up", function() { setLevel(true) });
		WindowKeys.keydownfunctions.setKey("down", function() { setLevel(false) });
		WindowKeys.keydownfunctions.setKey("left", function() { setSpeed(false) });
		WindowKeys.keydownfunctions.setKey("right", function() { setSpeed(true) });
		WindowKeys.keydownfunctions.setKey("space", function() { selectGame(brickGameModel.getGameNumber() + 1); });
		WindowKeys.keydownfunctions.setKey("enter", function() { GameSound.startGame(); Game.load(); }, false);
	...
	var Game = new function() {
		...
		this.lifeLost = function(again) {
			LifeBrickObject.removeTile(0, 4 - life);
			life--;
			if (life == 0) {
				gameOver();
			}
		...
		var lockKey = function(locked) {
			console.log("Timer Object length:", timers.length);
			WindowKeys.keydownfunctions.setKeyEnabled("up", !locked);
			WindowKeys.keydownfunctions.setKeyEnabled("down", !locked);
			WindowKeys.keydownfunctions.setKeyEnabled("left", !locked);
			WindowKeys.keydownfunctions.setKeyEnabled("right", !locked);
			WindowKeys.keydownfunctions.setKeyEnabled("space", !locked);
		};
		...
		function _load($game) {
			...
			// WindowKeys.keydownfunctions.deleteAllBut("s");
			WindowKeys.keydownfunctions.setKey("up", keydownfunctions.onTop.action, keydownfunctions.onTop.allowRepeat);
			WindowKeys.keydownfunctions.setKey("down", keydownfunctions.onBottom.action, keydownfunctions.onTop.allowRepeat);
			WindowKeys.keydownfunctions.setKey("left", keydownfunctions.onLeft.action, keydownfunctions.onTop.allowRepeat);
			WindowKeys.keydownfunctions.setKey("right", keydownfunctions.onRight.action, keydownfunctions.onTop.allowRepeat);
			WindowKeys.keydownfunctions.setKey("space", keydownfunctions.onSpace.action, keydownfunctions.onTop.allowRepeat);
			WindowKeys.keydownfunctions.defaultKeys(function() {}, false);
			WindowKeys.keydownfunctions.setKey("enter", function() {
				if (!isDead) {
	...
	var _games = [
	{
		...
		load: function() {
			...
			var spawnTankAnim = {};
			var tankTile = new BrickObject({ tiles: getBrickTiles("tankTile"), name: "tankTile", origin: { x: 1, y: 1 }, rotateDirection: "Left" });;

			// FUNCTIONS
			function initialize() {
				enemyTankTiles = [];
				removeBrickObjectsBut(obstacles, [tankTile]);
				spawnTankAnim = new Timer({ func: loadEnemyTanks, interval: 200 });
	...
	},
	{
		...
		load: function() {
			...
			function initialize() {
				...
				moveRoadAnim = new Timer({ func: moveRoad, interval: speedInMillis });
	...
	},
	{
		...
		load: function() {
			...
			function initialize() {
				...
				enemyInvasionAnim = new Timer({ func: invadeEnemies, interval: speedInMillis });
	...
	},
	{
		...
		load: function() {
			...
			function initialize() {
				...
				pinballThrowAnim = new Timer({ func: throwPinball, interval: speedInMillis });
			}
	...
	},
	{
		...
		load: function($game) {
			...
			function initialize() {
				...
				soldierObject.setLocation(18, 4);
				movePinballTileAnim = new Timer({ func: movePinballTile, interval: speedInMillis });
	...
	},
	{
		...
		load: function() {
			...
			function initialize() {
				...
				snakeCrawlAnim = new Timer({ func: crawlSnake, interval: speedInMillis });
	...
	var LifeBrickObject = new function() {
		...
		this.quadrupleTiles = function() {
			clearInterval(timer);
			show(0, 0, "quadrupleTiles");
		}
